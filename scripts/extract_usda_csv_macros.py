...
def write_output_csv(foods: list, output_path: Path, data_types: list = None) -> None:
    ...
    if data_types:
        ...
        logger.info(f"Filtered to {len(foods):,} foods of types: {', '.join(data_types)}")
    ...
    for food in foods[:10]:
        logger.info(f"  - {food['name']} ({food['data_type']}): "
...
def main():
    """Main entry point."""
    if len(sys.argv) < 2:
        print("Usage: python extract_usda_csv_macros.py <csv_directory> [output_csv_path] [--type TYPE1,TYPE2,...]")
        print("\nArguments:")
        print("  csv_directory     : Directory containing USDA CSV files")
        print("  output_csv_path   : Optional output CSV path (default: data/usda_all_macros.csv)")
        print("  --type            : Optional comma-separated list of data types to include")
        print("\nData types:")
        print("  - 'SR Legacy'     : Standard Reference (legacy) - best for basic ingredients (~7,000 foods)")
        print("  - 'Foundation'    : Foundation Foods - detailed nutrient data (~400 foods)")
