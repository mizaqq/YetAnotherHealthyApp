...
def process_usda_json(input_path: Path, output_path: Path) -> None:
    ...
    if extracted_foods:
        ...
        for food in extracted_foods[:5]:
            print(
                f"  - {food['name']}: {food['calories_kcal_per_100g']} kcal, "
...
def main():
    ...
    if len(sys.argv) < 2:
        print(
            "Usage: python extract_usda_macros.py <input_json_path> [output_csv_path]"
        )
        print("\nExample:")
        print(
            "  python extract_usda_macros.py ~/Downloads/FoodData_Central_foundation_food_json_2025-04-24.json"
        )
        sys.exit(1)
